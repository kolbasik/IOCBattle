...
namespace IocBattle.Benchmark.Tests
{
    public class DynamoTypeofContainer : IContainer
    {
        ...
        public void Setup(Container container)
        {
            ...
            _container.Register(c => new UserController(c.Resolve<IRepository>(), c.Resolve<IAuthenticationService>()));

            _container.Register<IWebService>(
                c =>
                    new WebService((IAuthenticator) c.Resolve(typeof (IAuthenticator)),
                        (IStockQuote) c.Resolve(typeof (IStockQuote))));
            _container.Register<IAuthenticator>(
                c =>
                    new Authenticator((ILogger) c.Resolve(typeof (ILogger)),
                        (IErrorHandler) c.Resolve(typeof (IErrorHandler)), (IDatabase) c.Resolve(typeof (IDatabase))));
            _container.Register<IStockQuote>(
                c =>
                    new StockQuote((ILogger) c.Resolve(typeof (ILogger)),
                        (IErrorHandler) c.Resolve(typeof (IErrorHandler)), (IDatabase) c.Resolve(typeof (IDatabase))));
            _container.Register<IDatabase>(
                c =>
                    new Database((ILogger) c.Resolve(typeof (ILogger)),
                        (IErrorHandler) c.Resolve(typeof (IErrorHandler))));
